...
- `~/.agent` 안의 파일 (`AGENTS.md`, `rules/*.md` 등) 을 수정·추가·삭제하는 작업을 마치면, 사용자에게 다시 묻지 않고 곧바로 `git add` → `git commit` → `git push origin main` 까지 수행한다. 이 저장소에 한해 커밋·푸시 권한은 상시 위임된 것으로 간주한다.
- 커밋 메시지는 변경 내용을 한 줄로 요약한다 (예: `chore: update style rule`).
- `.gitignore` 에 등재된 항목 (`.omc/`, `.DS_Store`) 은 커밋하지 않는다.
- 이 자동화는 `~/.agent` 디렉터리 내부 변경에만 한정된다. 다른 저장소의 커밋·푸시는 기존 원칙대로 사용자가 명시적으로 요청할 때만 한다.
